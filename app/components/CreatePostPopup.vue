<template>
    ...
    >
      ...
      >
        <h2 class="text-xl font-bold text-gray-800">
          {{ isHomepage ? 'Criar novo comunicado' : 'Criar novo post' }}
        </h2>
        <button
          class="text-gray-400 hover:text-gray-600 transition-colors bg-white rounded-full p-1 border border-gray-100 shadow-sm hover:shadow"
          @click="close"
        >
          <svg
      ...
      <div class="p-6 overflow-y-auto flex-1 custom-scrollbar space-y-5">
        ...
        <div>
          ...
          <input
            v-model="post.title"
            type="text"
            :placeholder="isHomepage ? 'Digite o título do comunicado...' : 'Digite o título do post...'"
        ...
        <div>
            ...
            >
              <div class="flex flex-col items-center justify-center pt-5 pb-6">
                ...
                <p class="mb-1 text-sm text-gray-500">
                  <span class="font-semibold">Clique para enviar</span> ou
                  arraste os arquivos
                </p>
                <p class="text-xs text-gray-500">
                  Imagens ou Vídeos (PNG, JPG, MP4...)
                </p>
              </div>
              <input
                id="dropzone-file"
          ...
          >
            <div
              ...
              :key="index"
              class="relative rounded-xl overflow-hidden border border-gray-200 shadow-sm aspect-square bg-black cursor-pointer hover:opacity-90 transition-opacity"
              @click="openPreview(fileItem)"
            >
              <img
                v-if="fileItem.type.startsWith('image/')"
                :src="fileItem.url"
                class="w-full h-full object-cover"
              >
              <video
                v-else-if="fileItem.type.startsWith('video/')"
                :src="fileItem.url"
                class="w-full h-full object-cover"
                muted
              />
              ...
              <button
                type="button"
                class="absolute top-1.5 right-1.5 bg-black/60 text-white p-1.5 rounded-full hover:bg-red-500 transition-colors shadow-sm"
                title="Remover mídia"
      ...
      >
        <button
          type="button"
          class="px-5 py-2.5 rounded-xl text-gray-700 bg-white border border-gray-200 hover:bg-gray-50 font-semibold transition-colors shadow-sm"
          @click="close"
        >
          Cancelar
        </button>
        ...
        >
          ...
          {{ isSubmitting ? 'Publicando...' : (isHomepage ? 'Publicar Comunicado' : 'Publicar Post') }}
        </button>
      </div>
    </div>
  </div>
  ...
  <div
    ...
    class="fixed inset-0 z-[60] bg-black/90 flex items-center justify-center p-4 backdrop-blur-sm"
    @click="closePreview"
  >
    <button
      class="absolute top-4 right-4 text-white hover:text-gray-300 transition-colors p-2 bg-black/50 rounded-full"
      @click="closePreview"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
    ...
    <img
      v-if="previewMediaType?.startsWith('image/')"
      :src="previewMediaUrl"
      class="max-w-full max-h-[90vh] object-contain rounded-lg"
      @click.stop
    >
    <video
      v-else-if="previewMediaType?.startsWith('video/')"
      :src="previewMediaUrl"
      controls
      class="max-w-full max-h-[90vh] object-contain rounded-lg"
      @click.stop
